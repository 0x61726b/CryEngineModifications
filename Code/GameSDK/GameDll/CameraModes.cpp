...
CIsometricCameraMode::CIsometricCameraMode()
{
}
...
bool CIsometricCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	const CCameraPose cameraPose = IsometricCameraPose(clientPlayer,  frameTime);
...
CCameraPose CIsometricCameraMode::IsometricCameraPose(const CPlayer& clientPlayer,float frameTime)
{
	...
	Vec3 isometricOffset = playerPos + Vec3(0,-12,10);
	Vec3 isometricCameraPose = isometricOffset;
	Quat isometricRotation = Quat::CreateRotationX(-45.0f * (gf_PI/180.0f));

	if(!bWorldSpace)
	{
		// Apply player rot onto pose for camera space rendering
		/*isometricCameraPose = clientPlayer.GetEntity()->GetWorldRotation() * isometricCameraPose;*/

	}

	const CCameraPose isometricCamera = CCameraPose(
		isometricCameraPose,
		isometricRotation);
...
bool CDefaultCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	Vec3 isometricCameraPose = isometricOffset;
	Quat isometricRotation = desiredLookAtOrientation;

	const CCameraPose cameraPose = CCameraPose(
		isometricCameraPose,
		isometricRotation);
