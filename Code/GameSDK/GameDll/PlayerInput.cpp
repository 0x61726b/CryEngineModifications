...
#undef ADD_HANDLER
	...
	m_pPlayer->SetThirdPerson(true,true);
	m_mouseX = 0;
	m_mouseY = 0;
	mouseDestination = Vec3(0,0,0);
...
bool CPlayerInput::OnActionAttack(EntityId entityId, const ActionId& actionId, int activationMode, float value)
{
	...
	if(activationMode == eAAM_OnRelease)
	{
		...
		CMovementRequest request;
		request.SetMoveTarget(m_Destination);
		request.SetStance(EStance::STANCE_STAND);

		request.SetDesiredSpeed(4.5f);

		request.SetAimTarget(m_Destination);
		request.SetLookTarget(m_Destination);
		request.SetBodyTarget(m_Destination);
		/*m_pPlayer->m_pMovementController->RequestMovement(request);*/

		const Vec3 lookAtEntityPosition = m_Destination;
		const Vec3 playerEyePosition = m_pPlayer->GetEntity()->GetWorldTM().TransformPoint(m_pPlayer->GetEyeOffset());
...
void CPlayerInput::UpdateEx()
{
	//Check distance
	Vec3 playerPosition = m_pPlayer->GetEntity()->GetWorldPos();

	const float valuesMagnitude = m_Destination.GetLength();
	const float movementEpsilon = valuesMagnitude * 0.000001f;

	Vec3 direction = m_Destination - playerPosition;
	float distance = direction.GetLength();



	bool stop = false;
	if(playerPosition.IsEquivalentL2(m_Destination,movementEpsilon) || distance < 1.0f) //temporary
	{
		stop = true;
...
void CPlayerInput::PostUpdateEx()
{
...
void CPlayerInput::CalculateDestination()
{
