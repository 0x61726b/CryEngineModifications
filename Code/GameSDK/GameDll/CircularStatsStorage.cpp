...
void CCircularBufferStatsContainer::Validate() const
{
	for (size_t i=0; i<m_numTimelines; i++)
	{
		...
		if (pTL->m_type==eRBPT_Invalid)
		{
			...
			{
				CryFatalError(string().Format("Memory stomp, stats container %p timeline %d has had nothing added to it, yet its head and tail have been altered (head %p , tail %p)",this,i,pHead,pTail));
			}
		}
		else
		{
			...
			{
				CryFatalError(string().Format("Memory stomp, stats container %p timeline %d has its head or tail out of the accepted range (head %p , tail %p)",this,i,pHead,pTail));
			}
		}
	}
}
